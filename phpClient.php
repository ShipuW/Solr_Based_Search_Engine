...
function excerpt($text, $phrase, $radius = 160, $ending = "...") { 
...
function getLinkfromMap($input) {
    $csv = array_map('str_getcsv', file('mapLATimesDataFile.csv'));
            ...
            <table class ="jsonTable">
                <li>
                    ...
                    <p><a href = "<?php if($id != '') {echo $id;} else {echo getLinkfromMap($MapFileIndex);}?>"> News Link </a> </br>
                        ...
                        Snippet :<?php
                        $local_link = "html/".$MapFileIndex;
                        $result = file_get_html($local_link, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT);
                        if ($result) {
                            $fileContent = $result->plaintext;
                        }else{
                            $fileContent = null;
                        }
                        ...
                        for ($i = 0; $i < $length; $i++) {
                            ...
                            if(stripos($value,$query)!== false) {
                                ...
                                echo  highlight(excerpt($value,$query),$query);
                                $flag ="true";
                                break;
