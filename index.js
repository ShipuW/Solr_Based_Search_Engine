            ...
            source: function(request, response) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var array = $.map(data.suggest, function(value, index) {
                            return value[extractLast( request.term.toLowerCase() ).trim()]["suggestions"];
                        });
                        ...
                        response($.map(rv, function(item) {


                            if((stopList.indexOf(item.replace(/[^a-zA-Z0-9/./_ ]/g, '')) == -1 && $('#q').val().indexOf(' ') == -1) || ($('#q').val().indexOf(' ') != -1)) {

                                if(unique_result.indexOf(stemmer(item.replace(/[^a-zA-Z/./_ ]/g, ''))) == -1 && i < 10) {
                                    unique_result.push(stemmer(item.replace(/[^a-zA-Z0-9/./_ ]/g, '')));
                                    i++;

                                    return {

                                        label: prefix + " " + item.replace("'s", "")
                                    }
                                }}
